...
URLS = [
    ...
    "https://companiesmarketcap.com/usa/largest-companies-in-the-usa-by-market-cap/?page=2"
]

# ---- Scraping ----
def scrape_symbols():
    """Scrape top 200 US companies with ticker and name (no price)."""
    symbols, names = [], []
...
def analyze(symbols, names):
    ...
    for idx, ticker in enumerate(symbols):
        try:
            ...
            if last_date == today:  # drop today’s partial data
                data = data.iloc[:-1]
...
# ---- Source Link ----
st.markdown("---")
st.markdown(f"[🔗 Source: CompaniesMarketCap.com]({URLS[0]})")
